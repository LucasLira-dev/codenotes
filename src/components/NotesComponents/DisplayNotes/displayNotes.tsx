...
export const DisplayNotes = ({ notesType, isLoading }: DisplayNotesProps) => {
    ...
    <article className="flex flex-col gap-4">
      {
        notesType === 'public' ? (
          publicNotes.length === 0 ? (
            <div className="flex flex-col items-center gap-3 mt-20 h-full">
              <NotesNotFound
              NotesType="public"
               />
            </div>
              ...
              <CardHeader>
                <div className="flex flex-col gap-3 min-w-0">
                  ...
                  <div className="flex flex-col gap-2 min-w-0">
                    <CardTitle className="text-[var(--foreground)] text-[18px] leading-normal truncate w-full">
                      {note.title}
                    </CardTitle>
                  ...
                  <CardHeader>
                    <div className="flex flex-col gap-3 min-w-0">
                      <div className="flex justify-between items-start w-full gap-4">
                        <div className="flex items-center gap-3 flex-1 min-w-0">
                          <Avatar className="h-10 w-10 flex-shrink-0">
                            <AvatarImage src={note.note.author.image ?? undefined} alt={note.note.author.name} />
                            <AvatarFallback className="bg-[var(--primary)] text-white text-sm">
                              {note.note.author.name.split(' ').map(n => n[0]).join('').toUpperCase()}
                            </AvatarFallback>
                          </Avatar>
                          <div className="min-w-0">
                            <p className="font-semibold text-[var(--foreground)] text-sm">
                              {note.note.author.name}
                            </p>
                            <p className="text-xs text-[var(--muted-foreground)]">
                        ...
                        <button
                          type="button"
                          onClick={() => addFavorite(note.note.id)}
                          className="flex items-center gap-2 px-3 py-2 rounded-lg bg-[var(--input)] hover:bg-[var(--muted)] transition-colors flex-shrink-0"
